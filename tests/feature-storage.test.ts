...
`;
  ...
  it("should parse taskType from frontmatter (UVS Phase 7)", () => {
    const content = `---
id: test.feature
taskType: ops
version: 1
...
`;
  ...
  it("should warn and skip invalid taskType", () => {
...
`;
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Invalid taskType"));

    warnSpy.mockRestore();
  });

  it("should parse verificationStrategies from frontmatter (UVS Phase 7)", () => {
    const content = `---
id: test.feature
version: 1
origin: manual
...
`;
    ...
    expect(feature.verificationStrategies).toBeDefined();
    expect(feature.verificationStrategies).toHaveLength(2);
    expect(feature.verificationStrategies![0].type).toBe("test");
    expect(feature.verificationStrategies![1].type).toBe("manual");
  });

  it("should warn and skip invalid verification strategies", () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
...
`;
    ...
    // Should only have 2 valid strategies (test and manual)
    expect(feature.verificationStrategies).toBeDefined();
    expect(feature.verificationStrategies).toHaveLength(2);
    expect(feature.verificationStrategies![0].type).toBe("test");
    expect(feature.verificationStrategies![1].type).toBe("manual");

    // Should have warned about invalid entries
    expect(warnSpy).toHaveBeenCalled();

    warnSpy.mockRestore();
  });

  it("should handle non-array verificationStrategies gracefully", () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
...
supersedes: []
tags: []
verificationStrategies: "not an array"
---
...
describe("serializeFeatureMarkdown", () => {
  ...
  it("should serialize UVS Phase 7 fields (taskType and verificationStrategies)", () => {
    const feature: Feature = {
      id: "test.uvs",
      description: "UVS fields test",
      module: "test",
      priority: 1,
      status: "failing",
      acceptance: ["Works"],
    ...
    expect(markdown).toContain("taskType: data");
    expect(markdown).toContain("verificationStrategies:");
...
describe("round-trip: parse(serialize(feature))", () => {
  ...
  it("should preserve taskType after round-trip (UVS Phase 7)", () => {
    const original: Feature = {
      id: "test.taskType",
      description: "TaskType round-trip test",
      module: "test",
      priority: 1,
      status: "failing",
      acceptance: ["TaskType should persist"],
      dependsOn: [],
      supersedes: [],
      tags: [],
      version: 1,
    ...
    expect(parsed.taskType).toBe("ops");
  });

  it("should preserve verificationStrategies after round-trip (UVS Phase 7)", () => {
    const original: Feature = {
      id: "test.verificationStrategies",
      description: "VerificationStrategies round-trip test",
      module: "test",
      priority: 1,
      status: "failing",
      acceptance: ["Strategies should persist"],
      dependsOn: [],
      supersedes: [],
      tags: [],
      version: 1,
    ...
    expect(parsed.verificationStrategies).toBeDefined();
    expect(parsed.verificationStrategies).toHaveLength(2);
    expect(parsed.verificationStrategies![0].type).toBe("test");
    expect(parsed.verificationStrategies![1].type).toBe("manual");
  });

  it("should preserve all UVS fields together after round-trip", () => {
    const original: Feature = {
      id: "test.allUvsFields",
      description: "All UVS fields round-trip test",
      module: "test",
      priority: 1,
      status: "failing",
      acceptance: ["All fields should persist"],
      dependsOn: [],
      supersedes: [],
      tags: [],
      version: 1,
      origin: "manual",
      notes: "Test notes for UVS fields.",
      taskType: "infra",
      verificationStrategies: [
        { type: "command", command: "terraform plan" },
        { type: "manual", description: "Infrastructure review" },
      ],
    };
    ...
    expect(parsed.taskType).toBe("infra");
    expect(parsed.verificationStrategies).toBeDefined();
    expect(parsed.verificationStrategies).toHaveLength(2);
    expect(parsed.verificationStrategies![0].type).toBe("command");
    expect(parsed.verificationStrategies![1].type).toBe("manual");
    expect(parsed.notes).toBe("Test notes for UVS fields.");
  });
});
...
describe("loadFeatureIndex", () => {
  ...
  it("should read ai/tasks/index.json and return FeatureIndex", async () => {
...
describe("saveFeatureIndex", () => {
  ...
  it("should use atomic write to prevent corruption", async () => {
    ...
    const files = await fs.readdir(path.join(tempDir, "ai/tasks"));
  ...
  it("should create directory structure if it doesn't exist", async () => {
    ...
    const exists = await fs
      .access(path.join(tempDir, "ai/tasks/index.json"))
      .then(() => true)
      .catch(() => false);
...
describe("loadSingleFeature", () => {
  ...
  it("should read ai/tasks/{module}/{name}.md", async () => {
...
Test feature.
`;
    await fs.mkdir(path.join(tempDir, "ai/tasks/cli"), { recursive: true });
    await fs.writeFile(
      path.join(tempDir, "ai/tasks/cli/survey.md"),
      featureContent
    );
...
describe("saveSingleFeature", () => {
  ...
  it("should create module directory if needed", async () => {
    ...
    const dirExists = await fs
      .access(path.join(tempDir, "ai/tasks/newmodule"))
      .then(() => true)
      .catch(() => false);
  ...
  it("should write markdown with YAML frontmatter", async () => {
    ...
    const content = await fs.readFile(
      path.join(tempDir, "ai/tasks/test/feature.md"),
      "utf-8"
    );
...
describe("migrateToMarkdown", () => {
  ...
  it("should create ai/tasks/ directory structure", async () => {
    await createLegacyFeatureList([]);

    await migrateToMarkdown(tempDir);

    const dirExists = await fs
      .access(path.join(tempDir, "ai/tasks"))
      .then(() => true)
      .catch(() => false);
    expect(dirExists).toBe(true);
  });
  ...
  it("should write each feature to {module}/{name}.md", async () => {
    ...
    const cliSurveyExists = await fs
      .access(path.join(tempDir, "ai/tasks/cli/survey.md"))
      .then(() => true)
      .catch(() => false);
    const authLoginExists = await fs
      .access(path.join(tempDir, "ai/tasks/auth/login.md"))
      .then(() => true)
      .catch(() => false);
