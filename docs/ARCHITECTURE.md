...
- **Path**: `src/index.ts`
- **Status**: complete
- **Description**: CLI entrypoint orchestrating analyze/init/next/status/impact/done/check/agents/scan commands with goal autodetect and progress output.
> CLI 入口，编排 analyze/init/next/status/impact/done/check/agents/scan 等命令，并具备目标自动检测与进度输出。
...
- **Path**: `src/gitignore/`
- **Status**: complete
- **Description**: Gitignore generator module with bundled templates (Node, Python, Go, Rust, Java, Nextjs), GitHub API integration with 7-day caching, and auto-detection from project config files.
> Gitignore 生成模块，包含内置模板及 GitHub API 集成（7天缓存），支持从项目配置文件自动检测。

### plugin-installer
- **Path**: `src/plugin-installer.ts`
- **Status**: complete
- **Description**: Auto-installs Claude Code plugins from embedded assets (binary mode) or local plugins directory (development mode).
> 从嵌入资源（二进制模式）或本地 plugins 目录（开发模式）自动安装 Claude Code 插件。

## Discovered Features

| ID | Description | Module | Source | Confidence |
|----|-------------|--------|--------|------------|
| cli.analyze | Generate AI-powered project survey and write docs/ARCHITECTURE.md with summary stats output.
> 生成 AI 驱动的项目调研并写入 docs/ARCHITECTURE.md，同时输出统计。 | cli | code | 90% |
...
| cli.check | Run feature verification without marking done, supporting optional AI mode (--ai), quick/full tests, and E2E control.
> 运行特性验证但不标记完成，支持可选 AI 模式 (--ai)、快速/完整测试与 E2E 控制。 | cli | code | 89% |
| cli.agents | List available AI agents and their availability.
> 列出可用的 AI 代理及其可用状态。 | cli | code | 84% |
| cli.scan | Scan project capabilities with optional verbose output.
> 扫描项目能力，支持详细输出。 | cli | code | 85% |
...
| ai.generateFromSurvey | Convert existing ARCHITECTURE.md and goal into feature list via AI.
> 通过 AI 将已有 ARCHITECTURE.md 与目标转化为特性清单。 | ai-scanner | code | 86% |
...
| features.loadSave | Read/write ai/tasks/ with schema validation and metadata updates.
> 读取/写入带架构校验和元数据更新的 ai/tasks/。 | feature-list | code | 88% |
...
| verification.autonomous | AI autonomous verification mode (enabled via --ai) that lets AI explore codebase with optional automated checks.
> AI 自主验证模式（通过 --ai 启用），可选运行自动检查后让 AI 自行探索代码库。 | verification | code | 88% |
...
- 3) Consider shipping example task templates and minimal fixtures to ease first-run experience and testing without AI.
> 3) 提供示例任务模板与最小示例，便于在无 AI 时快速上手与测试。
