/**
 * Verification result formatting and reporting
 */
...
export function formatVerificationResult(
  ...
  // Strategy results - show with clear optional/required distinction
  if (result.strategyResults && result.strategyResults.length > 0) {
    lines.push(chalk.bold("\n   Strategy Results:"));
    for (const sr of result.strategyResults) {
      const duration = sr.duration
        ? chalk.gray(` (${(sr.duration / 1000).toFixed(1)}s)`)
        : "";
      if (sr.success) {
        lines.push(`   ${chalk.green("✓")} ${sr.type.padEnd(10)} passed${duration}`);
      } else if (!sr.required) {
        lines.push(`   ${chalk.yellow("○")} ${sr.type.padEnd(10)} skipped ${chalk.gray("(optional)")}`);
      } else {
        lines.push(`   ${chalk.red("✗")} ${sr.type.padEnd(10)} failed${duration}`);
      }
    }
  }
